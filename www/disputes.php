...
<?
	...
	if($oldid == "1")
	{
	    ...
	    $res = mysql_query("select 1 from `email`, `users` where `email`.`email`='$email' and `email`.`memid`=`users`.`id` and `email`.`deleted`=0 and (`users`.`assurer_blocked`=1 or `users`.`locked`=1)");
		if(mysql_num_rows($res) > 0)
		{
			showheader(_("Email Dispute"));
